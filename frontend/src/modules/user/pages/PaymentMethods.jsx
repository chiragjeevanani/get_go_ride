...
const PaymentMethods = () => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 px-4">
      <header className="flex items-center gap-4 py-3 border-b-2 border-primary/20 sticky top-0 bg-white/80 backdrop-blur-md z-30 -mx-4 px-4 shadow-sm">
        ...
        <div className="flex flex-col">
          <h1 className="text-sm font-semibold text-black tracking-[0.1em] uppercase">Payments</h1>
      ...
      <div className="space-y-4">
        {cards.map((card) => (
          <Card key={card.id} className="border-none shadow-premium bg-white rounded-xl overflow-hidden relative">
            {card.isDefault && (
               <div className="absolute top-0 right-0 bg-primary text-black text-[8px] font-semibold uppercase px-3 py-1 rounded-bl-xl tracking-widest">
                  Default
               </div>
            )}
            <CardContent className="p-3">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col">
                    <span className="font-semibold text-black text-xs leading-none">{card.type}</span>
                ...
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="w-6 h-6 text-zinc-300 hover:text-red-500"
              ...
              <div className="mt-2 flex justify-between items-center bg-zinc-50/50 rounded-md p-1.5 px-2.5 border border-zinc-50">
                 <span className="text-[7px] font-semibold uppercase text-zinc-400 tracking-[0.1em]">Expiry</span>
                 <span className="text-[9px] font-semibold text-black tabular-nums">{card.expiry}</span>
              </div>
            </CardContent>
          </Card>
        ))}
