...
const RequestDetails = () => {
    ...
    try {
      ...
      setVendors((bidsRes.data || []).map(bid => {
        ...
        return {
          ...
          price: `₹${bid.amount}`,
          time: "Recently",
          avatar: v.profileImage || `https://api.dicebear.com/7.x/initials/svg?seed=${v.businessName || 'v'}`,
          bestValue: false
        };
      }));
  ...
  const timeline = [
    ...
    { 
      label: "Vendors Responding", 
      time: request?.status === 'Responding' || request?.status === 'Bidding' || request?.status === 'Pending' ? "Active" : "Completed", 
      done: !['Pending', 'Bidding', 'Responding', 'Open'].includes(request?.status), 
      current: ['Pending', 'Bidding', 'Responding', 'Open'].includes(request?.status) 
    },
    { 
      label: "Finalized", 
      time: request?.status === 'Finalized' || request?.status === 'Accepted' ? "Confirmed" : "Pending", 
      done: ['Finalized', 'Accepted', 'Completed'].includes(request?.status), 
      current: request?.status === 'Finalized' || request?.status === 'Accepted'
    },
  ];
                 ...
                 <CardContent className="p-4 space-y-4">
                    ...
                    <div className="flex items-center justify-between bg-primary/5 rounded-2xl p-3 border border-primary/10">
                       ...
                       <Button 
                          ...
                          onClick={() => navigate(`/user/vendor-profile/${vendor.vendorId}`, { state: { requestId: request._id } })}
                       >
