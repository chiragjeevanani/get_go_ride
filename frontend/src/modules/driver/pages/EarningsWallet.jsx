    ...
    <div className="min-h-screen bg-zinc-50 pb-24 max-w-md mx-auto">
      ...
      <header className="sticky top-0 z-40 bg-white border-b-2 border-yellow-400 px-4 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-semibold text-zinc-900 tracking-widest uppercase italic">Earnings Hub</h1>
            <p className="text-[8px] font-semibold text-zinc-400 tracking-widest uppercase mt-0.5">Financial Console</p>
          </div>
        </div>
        <Button variant="ghost" size="icon" className="w-9 h-9 rounded-xl border border-zinc-100" onClick={() => navigate("/driver/withdraw")}>
      ...
      <div className="p-4 space-y-6">
        {/* Main Balance Card */}
        <section>
          <Card className="bg-white border-2 border-zinc-100 rounded-xl overflow-hidden text-zinc-900 shadow-sm border-l-4 border-l-primary">
            <CardContent className="p-6 space-y-6 relative z-10">
              <div className="flex justify-between items-start">
                <div className="space-y-1">
                  <span className="text-[9px] font-semibold text-zinc-500 uppercase tracking-[0.3em]">Active Balance</span>
                  <h2 className="text-4xl font-semibold tracking-tighter tabular-nums text-zinc-900">
                    ₹{activeBalance.toLocaleString('en-IN')}
                  </h2>
                </div>
                <div className="w-12 h-12 bg-zinc-50 border border-zinc-100 rounded-xl flex items-center justify-center">
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-zinc-100 pt-5">
                <div className="space-y-1">
                  <span className="text-[8px] font-semibold text-zinc-600 uppercase tracking-widest flex items-center gap-1">
                    <Clock className="w-3 h-3" /> Pending
                  </span>
                  <p className="text-sm font-semibold text-amber-500">
                    ₹{pendingBalance.toLocaleString('en-IN')}
                  </p>
                  <p className="text-[7px] text-zinc-500 font-bold uppercase">Released after gig</p>
                </div>
                <div className="space-y-1 text-right">
                  <span className="text-[8px] font-semibold text-zinc-600 uppercase tracking-widest">Lifetime Pay</span>
                  <p className="text-sm font-semibold text-emerald-500">₹{totalEarnings.toLocaleString('en-IN')}</p>
                </div>
              </div>

              <div className="flex gap-2">
                <Button 
                  className="flex-1 h-10 bg-primary text-zinc-900 font-semibold text-[10px] uppercase tracking-wider rounded-xl hover:bg-yellow-400 shadow-sm"
                ...
                <Button
                  variant="outline"
                  className="flex-1 h-10 border-zinc-200 text-zinc-700 font-semibold text-[10px] uppercase tracking-wider rounded-xl hover:bg-zinc-50 hover:text-zinc-900 shadow-sm px-2"
            ...
            <div className="absolute bottom-0 right-0 w-32 h-32 bg-primary opacity-10 rounded-full blur-3xl"></div>
          </Card>
        </section>
        ...
        <section className="grid grid-cols-3 gap-2">
          {[
            ...
            <Card key={i} className="rounded-xl border-2 border-zinc-100 bg-white p-2.5 flex flex-col items-center gap-1">
              <stat.icon className="w-3.5 h-3.5 text-zinc-300" />
              <span className="text-xs font-semibold text-black leading-none">{stat.val}</span>
              <span className="text-[7px] font-semibold text-zinc-400 uppercase tracking-tighter">{stat.label}</span>
            </Card>
          ))}
        </section>
        ...
        {pendingBalance > 0 && (
          <section>
            <div className="bg-amber-50 border-l-4 border-amber-400 p-4 flex gap-3">
              ...
              <div>
                <p className="text-[10px] font-semibold text-amber-800 uppercase tracking-tight">Pending Balance: ₹{pendingBalance.toLocaleString('en-IN')}</p>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-[10px] font-semibold text-zinc-900 uppercase tracking-[0.2em] flex items-center gap-2 italic">
            ...
            <div className="flex gap-1 bg-zinc-100 p-1 overflow-x-auto">
              {["all", "advance_hold", "advance_release", "withdrawal"].map(tab => (
                <button
                  ...
                  className={cn(
                    "px-2 h-6 text-[7px] font-semibold uppercase tracking-wider transition-all whitespace-nowrap",
          ...
          <div className="space-y-2.5">
            {filteredTxns.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">No transactions yet</p>
              </div>
            ) : (
              filteredTxns.map((item, idx) => {
                ...
                return (
                  <Card key={idx} className="border-2 border-zinc-100 rounded-xl shadow-none hover:border-primary/40 transition-all bg-white">
                    <CardContent className="p-3.5 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-[11px] font-semibold text-zinc-900 uppercase tracking-tighter">{config.label}</h4>
                      ...
                      <span className={cn("text-[13px] font-semibold tabular-nums", isPositive ? "text-emerald-500" : "text-zinc-900")}>
