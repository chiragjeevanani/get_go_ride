...
const resolveBid = async (params, user) => {
  const { bidId, requestId, vendorId } = params;

  let bid = null;
  if (bidId) {
    // Try finding by direct Bid ID first
    ...
    // If not found, check if bidId is actually a Requirement ID
    if (!bid && user) {
      if (user.role === 'vendor') {
        // For driver/vendor, find their bid on this requirement
        bid = await Bid.findOne({ requirement: bidId, vendor: user.id })
          .populate({
            path: 'requirement',
            populate: { path: 'user', select: 'name phone profileImage' }
          })
        ...
        // For customer user, find any active bid/negotiation on this requirement
        bid = await Bid.findOne({ requirement: bidId })
          .populate({
            path: 'requirement',
            populate: { path: 'user', select: 'name phone profileImage' }
          })
  ...
  } else if (requestId && vendorId) {
    bid = await Bid.findOne({ requirement: requestId, vendor: vendorId })
      .populate({
        path: 'requirement',
        populate: { path: 'user', select: 'name phone profileImage' }
      })
